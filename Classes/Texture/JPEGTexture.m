...
-(Texture*)PreinitWithData:(NSData*)inData textureParams:(TextureParams*)inParams
{
    ...
    FILE* memoryFile = fmemopen((void*)[inData bytes], [inData length], "rb");
    
    jpeg_stdio_src(&cinfo, memoryFile);
    
    jpeg_read_header(&cinfo, TRUE);
    
    jpeg_start_decompress(&cinfo);

    NSAssert(cinfo.output_components == 4 || cinfo.output_components == 3, @"Unexpected number of components (only RGBA is supported)");
    ...
    mTexBytes = (u8*)malloc(mWidth * mHeight * cinfo.output_components);
    
    int rowBytes = cinfo.output_width * cinfo.output_components;

    while (cinfo.output_scanline < cinfo.output_height)
    {
        u8* bufPtr = &mTexBytes[rowBytes * cinfo.output_scanline];
...
{
    [self PreinitWithData:inData textureParams:inParams];
    [self CreateGLTexture];
...
{
    NSAssert(FALSE, @"JPEG textures don't support mipmaps");
